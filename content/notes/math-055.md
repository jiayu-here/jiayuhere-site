...
slug: math-055
description: "考研数学学习笔记：Lecture 53：常数项级数。保留原始公式、图示与例题。"
category: "数学"
date: 2024-02-12
tags: ["考研数学", "高等数学", "无穷级数"]
---
...
### 47.1.2 级数的性质 
**基本性质**
- 性质 1：若 $\sum_{n=1}^{\infty}u_n$ 收敛于 $S$，则 $\sum_{n=1}^{\infty}k u_n$ 收敛于 $kS$。
- 性质 2：
	- 若 $\sum_{n=1}^{\infty}H_{n}$ 和 $\sum_{n=1}^{\infty}U_{n}$ 分别收敛于 $h$ 和 $u$，则 $\sum_{n=1}^{\infty}(H_{n}\pm U_{n})$ 收敛于 $h\pm u$。
	- 两个收敛的级数，相加或相减后仍然收敛；
...
**分类**
- 同号级数
	- 正项级数 `->` [Lecture 48：正项级数及其审敛准则](/notes/math-056/)
	- 负项级数：就是正项级数的基础上、加一个负号；
- 变号级数：
	- 特殊的变号级数：交错级数 `->` [Lecture 49：交错级数与任意项级数](/notes/math-057/)
	- 一般的变号级数：任意项级数 `->` [Lecture 49：交错级数与任意项级数](/notes/math-057/)
