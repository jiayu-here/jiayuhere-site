...
slug: math-010
description: "考研数学学习笔记：Lecture 2：函数的概念及常见函数。保留原始公式、图示与例题。"
category: "数学"
date: 2024-02-26
...
### 2.1.1 函数的定义
##### **定义**： #函数
> <font color="#ccc1d9">描述：</font>如果对于每个数 $x\in D$，按照一定的法则都有唯一确定的 $y$ 与之对应，则称 $y$ 是 $x$ 的函数，记为 $y=f(x)$；通常称 $x$ 为自变量，$y$ 为因变量，$D$ 为定义域。
...
### 2.1.2 其他函数类型
**取整函数**
- 设 $x$ 为任意实数，不超过 $x$ 的最大整数称为 $x$ 的整数部分，记为 $[x]$。
- 函数 $y=[x]$ 称为取整函数；

## 2.2 其他常见函数
### 2.2.1 复合函数
##### **定义**： #复合函数
> <font color="#ccc1d9">描述：</font>设 $y=f(u)$ 的定义域为 $D_f$，$u=g(x)$ 的定义域为 $D_g$、值域为 $R_g$。若 $D_f\cap R_g\neq\varnothing$，则称函数 $y=f[g(x)]$ 为函数 $y=f(u)$ 与 $u=g(x)$ 的复合函数。它的定义域为 $\left\{x\mid x\in D_g,\ g(x)\in D_f\right\}$。
...
**易错点**：注意复合函数中的值域与定义域 
- 概念： 
	- 复合函数的定义域是 $\left\{x\mid x\in D_g,\ g(x)\in D_f\right\}$。
- 易错点：
	- 把内层函数的定义域当成复合函数的定义域；

### 2.2.2 反函数
##### **定义**： #反函数
> <font color="#ccc1d9">描述：</font>设函数 $y=f(x)$ 的定义域为 $D$，值域为 $R_f$。若对任意 $y\in R_f$，都有唯一确定的 $x\in D$ 使得 $y=f(x)$，则由 $x=f^{-1}(y)$ 所确定的函数称为 $y=f(x)$ 的反函数。

**解释**
- 普通函数允许多个不同的 $x$ 对应同一个 $y$；
- 原函数存在反函数的条件是它在定义域上为一一映射，即不同的 $x$ 对应不同的 $y$；
- 概念：
	- 1. $\text{有时也将}\quad y=f(x)\text{ 的反函数 }x=f^{-1}(y)\text{ 写成 }y=f^{-1}(x)$
	- 2. $\text{在同一直角坐标系中，}y=f(x)\text{ 和 }x=f^{-1}(y)\text{ 的图形重合}$；
...
### 2.2.3 反函数和复合函数结合
**两者结合**：
- 情况一：当 $x\in D$ 时，$f^{-1}[f(x)]=x$
	- 普通函数传入到当前函数的反函数当中
- 情况二：当 $x\in R_f$ 时，$f[f^{-1}(x)]=x$
