...
> 	二阶微分方程一般形式：$y^{\prime\prime}=f(x,y,y^{\prime})$
...
- 举例：研究小球往上抛出去后，其位置变化；
	- 当前的微分方程：二阶导数
		- 小球的重力加速度
			- $$\ddot{y}(t)=-g$$
	- 知道**一阶导数**：
		- 知道向下的分量为重力加速度 $-g$，因此 $\ddot{y}(t)=-g$，还可以将其和速度的关系描述出来：$\frac{d}{dt}(-gt+v_0)=-g$。
		- 但只知道这个还无法确定原函数，还需要为其增加初始状态，来进行约束；
	- 加上**初始条件 1**：
		- 在 $v_0$ 的速度基础上，进行的向下 g 作用：$\dot{y}(t)=-gt+v_0$
		- 问题：仍不知道原函数的具体形式，只能写成 $\frac{d\,y(t)}{dt}=-gt+v_0$。
	- 知道**原函数**：
		- $\frac{d}{dt}\left(-\frac{1}{2}gt^{2}+v_{0}t\right)=-gt+v_{0}$。
	- 加上**初始条件 2**：
		- 当物体的初始位置为 $y_0$ 时，且其关系为：$\frac{d}{dt}\left(-\frac{1}{2}gt^{2}+v_{0}t+y_{0}\right)=-gt+v_{0}$。
	- 得到**微分方程的解**：
...
- 一元方程：
	- 一阶方程： 
		- （A）齐次方程：
			...
			- 方法：令 $u=\frac yx$，代入并整理 `->` 化为可分离变量方程；
		- （B）线性方程：
			- 形式：$$y^{\prime}+P(x)y=Q(x)$$
			- 方法：整理成标准形式，然后代入通解公式 $$y=e^{-\int p(x)dx}\left[\int Q(x)e^{\int p(x)dx}dx+C\right]$$ `->` [Lecture 36：一阶线性微分方程](/notes/math-044/)
		- （C）伯努利方程：
	...
	- 二阶方程：判断系数
		- 常系数：
			- （D）二阶常系数齐次方程：
				...
				- 方法：写特征方程，找特征根 `->` [Lecture 38：常系数齐次线性微分方程](/notes/math-046/)
			- （E）二阶常系数非齐次方程： 
				- 形式：$$y^{\prime\prime}+py^{\prime}+qy=f(x)$$
				- 方法：将非齐次方程分成两个部分，即**非齐次方程通解 = 对应齐次方程通解 + 非齐次方程的一个特解**； `->` [Lecture 39：常系数非齐次线性微分方程](/notes/math-047/)
		- 变系数：
			- （F）可降阶方程：有 x 有 y
...
- 2. 求微分方程的反问题：知道一个特解，求微分的方程；
	- 分析：特解 = 通解把某个条件定下来 
	- 比如二阶常系数非齐次方程中：非齐次方程通解 = 对应齐次方程通解 + 一个非齐次特解，即 $y=C_{1}y_{1}+C_{2}y_{2}+y^{\star}$
