...
slug: math-058
description: "考研数学学习笔记：Lecture 56：幂级数。保留原始公式、图示与例题。"
category: "数学"
date: 2024-02-14
tags: ["考研数学", "高等数学", "无穷级数"]
---
...
> 3. $S\left (x\right)=u_1\left (x\right)+u_2\left (x\right)+\cdots+u_n\left (x\right)+\cdots$ 称为和函数，$\lim_{n\to\infty}S_n(x)=S\left(x\right)$；
...
- 此时：
	- 当 $|x|<1$ 时，收敛域为 $(-1,1)$，和函数为 $\frac{1}{1-x}$；
	- 当 $|x|\geq1$ 时，级数发散，发散域为 $(-\infty,-1]\cup[1,+\infty)$；
...
- 概念：
	- 收敛区间： 
		- 对 $\sum_{n=0}^{+\infty}a_n(x+2)^n$ 而言，其中心在 `x=-2` 点。若收敛半径为 `2`，则其开收敛区间为：
