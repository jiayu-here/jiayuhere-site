...
- 分析
	...
	- 第一步：它的范围；是 ![Pasted image 20240131150018](/assets/notes/86669d54a8c5-Pasted-image-20240131150018.png)
...
- 解析：第一问
	...
	- 求下半部分圆的面积微分：
		- 假设现在从 $y$ 到 $y+dy$ 切出一个很薄的圆盘，此时圆盘的截面积为 $\pi x^2$；
		- 乘以厚度 $dy$ 后，$\pi x^2\,dy$ 就是这一薄层的体积微元；
	- 然后将体积微元在 $-1$ 到 $\frac12$ 上积分：
		- $\pi\int_{-1}^{\frac{1}{2}}x^{2}\operatorname{d}y$
	- 再乘以 2，并代入 $x^2=1-y^2$，得到：
		- $V=2\pi\int_{-1}^{\frac{1}{2}}x^{2}\mathrm{d}y=2\pi\int_{-1}^{\frac{1}{2}}(1-y^{2})\mathrm{d}y=\frac{9\pi}{4}$
- 解析：第二问 
	- 因为力的大小不变，因此 功 = 力 × 位移
		- 每一薄层所受的重力随该层体积变化，因此需要按高度积分；
		- 薄层的重力等于体积乘以密度再乘以 $g$；
	- 位移：$2-y$
