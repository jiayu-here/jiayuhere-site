...
- 核心：
	...
	- 可以用通用方法降阶、求解的两种类型：$y^{\prime\prime}=f(x,y^{\prime})$ 和 $y^{\prime\prime}=f(y,y^{\prime})$
	- 判断所属的可降阶类型，进行对应的代换，然后使用**分离变量法**；
- 注意：
	- 求高阶微分方程的定解时，每次积分都会产生一个常数，再用初始条件或边界条件逐一确定；

**常见可降阶形式**
- 1. $y^{\prime\prime}=f(x)$；
	- 举例：$y^{\prime\prime}=e^{x},\quad y^{\prime}=e^{x}+C_{1},\quad y=e^{x}+C_{1}x+C_{2}$
...
##### **定理**： #二阶线性非齐次微分方程的解
> <font color="#8db3e2"><font color="#c6d9f0">描述：</font></font>如果 $y^*$ 是二阶线性**非齐次**微分方程的一个特解，$y_1(x)$ 和 $y_2(x)$ 是对应二阶线性**齐次**微分方程的两个线性无关解，则：
...
##### **定理**： #非齐次微分方程与齐次微分方程的解
> <font color="#8db3e2"><font color="#c6d9f0">描述：</font></font>如果 $y^*_1(x)$ 和 $y^*_2(x)$ 是同一个非齐次微分方程的两个特解，则 $y^*_1(x)-y^*_2(x)$ 是对应**齐次**微分方程的解；
...
> $$y^*_1(x)+y^*_2(x)$$ 是方程 $y^{\prime\prime}+p(x)y^{\prime}+q(x)y=f_1(x)+f_2(x)$ 的一个特解；
