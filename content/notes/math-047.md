...
slug: math-047
description: "考研数学学习笔记：Lecture 38：常系数非齐次线性微分方程。保留原始公式、图示与例题。"
category: "数学"
date: 2024-02-02
tags: ["考研数学", "高等数学", "微分方程"]
---
...
- 三种待定特解：
	...
	- 情况 2：若非齐次项是 $f(x)=e^{\alpha x}[P_l(x)\cos\beta x+Q_n(x)\sin\beta x]$
		- 即非齐次项是 $指数×多项式×三角$；
...
- 4. 求待定特解各项： 
	...
	- $x^{k}$ 
		...
		- 如果是情况二：
			...
			- 注意：若 $\beta\neq0$，则 $\alpha\pm i\beta$ 是一对共轭复根；$k$ 取其中任一根在特征方程中的重数；
- 5. 确定系数与任意常数
	- （1）确定非齐次项待定特解的系数
	...
	- （2）确定齐次项通解的任意常数 $C_1$ 和 $C_2$
		- 根据题目给出的两个初始条件，例如 $y(0)=0$、$y^{\prime}(0)=1$；
